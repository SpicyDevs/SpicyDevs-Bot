...
module.exports = async (client) => {
    ...
    client.loadSubcommands = async function (client, interaction, args) {
        try {
            ...
            if (data.Beta == true) {
                return require(`${process.cwd()}/src/commands/slash/${interaction.commandName}/${interaction.options.getSubcommand()}-beta`)(client, interaction, args).catch(err => {
                    client.emit("errorCreate", err, interaction.commandName, interaction)
                })
            }
            else {
                return require(`${process.cwd()}/src/commands/slash/${interaction.commandName}/${interaction.options.getSubcommand()}`)(client, interaction, args).catch(err => {
                    client.emit("errorCreate", err, interaction.commandName, interaction)
                })
            }
        }
